...
async function sendMail(senderId , reciverId , sentData , reciveData ,setSending){
  ...
  setSending(true);
    try {
      //sentdata
      const responseToSender = await fetch(`https://loginpage-ff00d-default-rtdb.firebaseio.com/mailbox/
      ...
      //rescive data
      await fetch(`https://loginpage-ff00d-default-rtdb.firebaseio.com/mailbox/
      ...
      // If the requests are successful, setSending is called with a value of false to indicate that the mail has been sent, and the function returns the data from the first fetch() request.
